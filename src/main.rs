...
async fn main() {
    ...
    sqlx::query("CREATE TABLE IF NOT EXISTS boot (started_at INTEGER NOT NULL)")
        .execute(&db)
        .await
        .expect("boot table");
    sqlx::query("INSERT INTO boot (started_at) VALUES (?)")
        .bind(now_secs() as i64)
        .execute(&db)
        .await
        .expect("boot row");
    info!(
        config = "generated in-memory room store; supplied PORT if set",
        build_sha = BUILD_SHA,
    ...
    let not_found = ServeFile::new(dist.join("404.html"));
    let app = Router::new()
...
async fn security_and_rate_limit(
    ...
    if request.uri().path() != "/health" {
        ...
        let entry = limits.entry(key).or_insert(LimitWindow {
            started: Instant::now(),
            count: 0,
        });
        if entry.started.elapsed() >= Duration::from_secs(1) {
    ...
    } else if request_path.ends_with(".avif")
        || request_path.ends_with(".webp")
        || request_path.ends_with(".jpg")
...
async fn create_room(
    ...
    let room = Room {
        ...
        phase: 0,
        total_turns: if input.paid { 8 } else { 4 },
        deadline: None,
        strokes: vec![],
        snapshots: vec![],
        guesses: vec![],
    ...
    (
        ...
        Json(RoomCredentials {
            ...
            token: host_token,
            role: "host",
            expires_at: now_secs() + ROOM_TTL.as_secs(),
        }),
    )
}
...
async fn get_room(
    ...
    let Some(room) = rooms.get(&normalize_code(&code)) else {
        return error(
            StatusCode::NOT_FOUND,
            "Room not found. Ask the host to make a new room.",
        );
    };
    let Some(role) = role_for(room, &auth.token) else {
        return error(
            StatusCode::UNAUTHORIZED,
            "This private room link is not valid. Open your latest invite link.",
        );
    };
...
async fn ws_handler(
    ...
    let role = {
        ...
        rooms
            .get(&code)
            .and_then(|room| role_for(room, &auth.token))
            .map(str::to_string)
    };
    let Some(role) = role else {
        return error(
            StatusCode::UNAUTHORIZED,
            "This private room link is not valid.",
        );
    };
...
async fn handle_event(state: &AppState, code: &str, token: &str, event: ClientEvent) {
    ...
    if let Some(room) = rooms.get_mut(code) {
        let Some(role) = role_for(room, token) else {
            return;
        };
        ...
        match event {
            ...
            ClientEvent::EndRoom if role == "host" => {
                let _ = room
                    .tx
                    .send(serde_json::json!({"kind":"ended"}).to_string());
                remove = true;
            }
            ClientEvent::Ping => {}
            _ => {}
...
fn advance(room: &mut Room) {
    ...
    room.phase += 1;
    room.deadline = if room.phase < room.total_turns {
        Some(now_secs() + TURN_SECONDS)
    } else {
        None
    };
}
...
fn broadcast_views(room: &Room) {
    let _ = room
        .tx
        .send(serde_json::to_string(&view(room, "host")).unwrap());
    let _ = room
        .tx
        .send(serde_json::to_string(&view(room, "guest")).unwrap());
}
...
fn view(room: &Room, role: &str) -> RoomView {
    RoomView {
        ...
        role: role.into(),
        partner_connected: if role == "host" {
            room.guest_connected
        } else {
            room.host_connected
        },
        phase: room.phase,
...
fn room_ttl_remaining(room: &Room) -> u64 {
    ROOM_TTL.saturating_sub(room.created.elapsed()).as_secs()
}
...
fn spawn_timer(state: AppState, code: String) {
    tokio::spawn(async move {
        loop {
            ...
            if room.phase >= room.total_turns {
                break;
            }
            if room.deadline.is_some_and(|d| d <= now_secs()) {
                advance(room);
                broadcast_views(room);
            }
        }
...
async fn prune_rooms(state: &AppState) {
    state
        ...
        .lock()
        .await
        .retain(|_, room| room.created.elapsed() < ROOM_TTL);
}
