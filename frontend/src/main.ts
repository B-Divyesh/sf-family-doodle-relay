...
function resultHtml(state:RoomState){return `<section class="result-strip"><p class="kicker">The final edition</p><h2>From one line to a family story</h2><div class="result-panels">${state.snapshots.map((_,i)=>`<canvas class="result-canvas" data-index="${i}" width="640" height="480" aria-label="Drawing from relay turn ${i+1}"></canvas>`).join('')}</div>${state.guesses.map((g,i)=>`<p class="result-quote">Turn ${i+1} guess: “${escapeHtml(g)}”</p>`).join('')}<div class="tools"><button class="primary" id="download-strip">Download the PNG strip</button><button id="new-room">Make another room</button></div><p class="muted">The download is made on this device.</p></section>`;}
