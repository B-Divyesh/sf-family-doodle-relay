...
function header() {
  return `<div id="route-announcer" class="sr-only" aria-live="polite"></div>
  <header class="site-header"><a class="skip" href="#main">Skip to main content</a><a class="wordmark" href="/" data-link>Family Doodle Relay</a>
...
function footer() {
  ...
  <div class="footer-links"><a href="/privacy" data-link>Privacy</a><a href="/terms" data-link>Terms</a><a href="https://hello-factory.sociobot.in">Built by Param Factory <span aria-label="external link">↗</span></a><span>v1.0.3</span><span>Art generated for this product</span></div></footer>`;
}
...
function setMeta(title: string, description: string, canonicalPath: string) {
  ...
  document.querySelector<HTMLMetaElement>('meta[property="og:url"]')!.content = `https://family-doodle-relay.sociobot.in${canonicalPath}`;
...
function landing() {
  ...
  app.innerHTML = `${header()}<main id="main" class="route-enter">
    <section class="hero"><div class="hero-copy"><h1 tabindex="-1">Draw together from two places</h1>
    <p class="lede">For a child and one trusted adult who want a calm game between calls.</p>
    <div class="hero-actions"><a class="button primary" href="/?demo=1" data-link>Try it with sample data</a><a class="button" href="/play" data-link>Make a private room</a><p class="after-action">A sample relay opens next. Nothing is saved.</p></div>
    <ul class="facts"><li>Two people only</li><li>Rooms close within four hours</li><li>$6 once, no subscription</li></ul></div>
    <figure class="hero-art"><picture><source media="(max-width: 700px)" type="image/avif" srcset="/relay-hero-mobile.avif"><source media="(max-width: 700px)" type="image/webp" srcset="/relay-hero-mobile.webp"><source type="image/avif" srcset="/relay-hero.avif"><source type="image/webp" srcset="/relay-hero.webp"><img src="/relay-hero.jpg" width="1200" height="800" alt="Two hands pass a folded drawing of houses becoming a whale." fetchpriority="high"></picture><figcaption>A shared drawing moves between two people.</figcaption></figure></section>
    ...
    <section class="section" aria-labelledby="preview-title"><div class="wrap"><div class="section-heading"><h2 id="preview-title">Sample relay</h2><p>The timer keeps each turn short. The finished strip keeps every surprising turn together.</p></div>
    <div class="preview-paper"><div class="preview-rail"><div class="edition">Sample relay · 00:34 left</div><p class="kicker">Turn three of four</p><h3>Add one detail</h3><p>Sam guessed “a house at sea.” Add to the same drawing.</p></div><div class="preview-drawing" aria-label="Sample drawing of a house and whale"><svg viewBox="0 0 600 320" role="img" aria-label="A loose line drawing of a house sailing on a whale"><path d="M90 210 Q150 110 230 210 L230 265 L90 265 Z M130 180v-42h58v72 M325 225q80-115 160 0q-25 43-80 35q-55 8-80-35 M470 206q45-20 53-67q-8 45 30 67 M356 226q-28 28-50 4" fill="none" stroke="#171714" stroke-width="7" stroke-linecap="round" stroke-linejoin="round"/><circle cx="453" cy="219" r="5" fill="#171714"/><path d="M420 176q8-30 20 0" fill="none" stroke="#9c2f24" stroke-width="6"/></svg></div></div></div></section>
    <section class="section" id="how" aria-labelledby="how-title"><div class="wrap"><div class="section-heading"><h2 id="how-title">How the relay works</h2><p>No account is needed. Share one private link with the person you know.</p></div><div class="steps"><article class="step"><span class="step-no">01</span><h3>Make a room</h3><p>Send its private invite to one person.</p></article><article class="step"><span class="step-no">02</span><h3>Take four turns</h3><p>Draw, guess, then add one surprising detail.</p></article><article class="step"><span class="step-no">03</span><h3>Save the PNG strip</h3><p>Download the finished relay as one PNG strip.</p></article></div></div></section>
    <section class="section" aria-labelledby="privacy-title"><div class="wrap privacy-grid"><div><h2 id="privacy-title">Private rooms and data</h2><p>Rooms disappear from the server within four hours. Downloading a PNG strip does not send it to another service.</p></div><div><h3>What this does not have</h3><ul class="not-list"><li>Public rooms or strangers</li><li>Profiles or follower counts</li><li>Ads or behaviour tracking</li><li>Open text chat</li></ul></div></div></section>
    <section class="section" aria-labelledby="paid-title"><div class="wrap privacy-grid"><div><h2 id="paid-title">Family edition: eight-turn rooms</h2><p class="price-line">$6 once</p><p>Eight-turn rooms are included. Core four-turn play and PNG strips stay free.</p><a class="button primary" href="https://api.sociobot.in/api/v1/products/family-doodle-relay/checkout">Buy the family edition</a><p class="muted">One-time purchase. Payment opens on Sociobot.</p></div>
...
function demoBanner() {
  return `<section class="demo-banner" aria-label="Demo mode"><strong>Demo — sample data, nothing is saved</strong><div><button id="reset-demo" type="button">Reset demo</button><a href="/" data-link>Start for real</a></div></section>`;
}
...
function inviteHtml(code:string){const url=`${location.origin}/join/${code}`;return `<div class="invite-panel"><h2>Invite one person</h2><p>Send this private link to your child or trusted adult.</p><code class="invite-url">${escapeHtml(url)}</code><div class="tools"><button id="copy-invite">Copy the invite link</button><button id="end-room" class="danger">End this room</button></div></div>`;}
...
function bindGuess(active:boolean,send:(event:any)=>void){if(!active)return;document.querySelector<HTMLFormElement>('#guess-form')?.addEventListener('submit',(event:SubmitEvent)=>{event.preventDefault();const form=event.currentTarget as HTMLFormElement;const input=form.elements.namedItem('guess') as HTMLInputElement;const guess=input.value.trim();if(!guess){document.querySelector('#guess-help')!.textContent='Write a guess before sending it.';input.focus();return;}send({type:'submit_guess',guess});});}
...
function resultHtml(state:RoomState){return `<section class="result-strip"><p class="kicker">Finished relay</p><h2>Download your relay</h2><div class="result-panels">${state.snapshots.map((_,i)=>`<canvas class="result-canvas" data-index="${i}" width="640" height="480" aria-label="Drawing from relay turn ${i+1}"></canvas>`).join('')}</div>${state.guesses.map((g,i)=>`<p class="result-quote">Turn ${i+1} guess: “${escapeHtml(g)}”</p>`).join('')}<div class="tools"><button class="primary" id="download-strip">Download the PNG strip</button><button id="new-room">Make another room</button></div><p class="muted">The PNG is created here and is not sent to another service.</p></section>`;}
...
function downloadStrip(state:RoomState){const frames=state.snapshots.length?state.snapshots:[state.strokes];const columns=2;const rows=Math.ceil(frames.length/columns);const canvas=document.createElement('canvas');canvas.width=1200;canvas.height=170+rows*390+Math.max(1,state.guesses.length)*44+80;const context=canvas.getContext('2d')!;context.fillStyle='#f5f0e5';context.fillRect(0,0,canvas.width,canvas.height);context.fillStyle='#171714';context.font='bold 58px Georgia';context.fillText('Our Family Doodle Relay',60,80);context.font='18px Arial';context.fillText(`Edition ${new Date().toISOString().slice(0,10)} · ${state.total_turns} turns`,60,118);frames.forEach((strokes,index)=>{const temp=document.createElement('canvas');temp.width=500;temp.height=360;drawStrokes(temp,strokes);const x=60+(index%columns)*550;const y=155+Math.floor(index/columns)*390;context.drawImage(temp,x,y,500,360);context.strokeRect(x,y,500,360);});context.font='italic 24px Georgia';const quoteTop=155+rows*390;state.guesses.forEach((guess,index)=>context.fillText(`Turn ${index+1}: “${guess.slice(0,68)}”`,60,quoteTop+index*40));context.font='16px Arial';context.fillText('Made with Family Doodle Relay',60,canvas.height-36);const link=document.createElement('a');link.download='family-doodle-relay.png';link.href=canvas.toDataURL('image/png');link.click();}
...
function legalPage(kind:'privacy'|'terms'){
  ...
  app.innerHTML=`${header()}<main id="main" class="legal route-enter"><article class="narrow"><p class="kicker">Last updated 30 August 2026</p><h1 tabindex="-1">${privacy?'Privacy in plain words':'Terms for family play'}</h1>${privacy?`<p>We made this game for a child and one trusted adult. It does not need a profile.</p><h2>What a room holds</h2><p>The server stores the room code, access-key hashes, turn and timer state, drawing panels, and guesses in SQLite. It also stores creation, expiry, and brief presence times. It removes the room within four hours.</p><h2>What stays on your device</h2><p>This browser keeps your private room key and optional license in local storage. A PNG strip is created and downloaded on your device.</p><h2>Who receives data</h2><p>Restoring a license sends one check to <strong>api.sociobot.in</strong>. The request includes the license token and no room data. We do not use ads or behaviour tracking.</p><h2>Children</h2><p>An adult should share the private invite only with someone they trust.</p><h2>Contact</h2><p>For privacy requests, email <a href="mailto:privacy@sociobot.in">privacy@sociobot.in</a>.</p>`:`<p>Use this service only with a person you know. A host may end a room at any time.</p><h2>Use of the game</h2><p>Do not share illegal, abusive, or identifying material. You are responsible for the drawings and guesses you add.</p><h2>Room availability</h2><p>Rooms close within four hours. Download a PNG strip before leaving if you want to keep it.</p><h2>One-time purchase</h2><p>The $6 family edition adds eight-turn rooms. It is a one-time purchase. Payment opens on Sociobot.</p><p>Dodo Payments is the merchant of record. Its checkout handles order questions and returns. A refunded license cannot enable eight-turn rooms.</p><h2>No warranty</h2><p>The service is provided as available. These terms do not remove rights that consumer law gives you.</p><h2>Contact</h2><p>For terms or purchase questions, email <a href="mailto:support@sociobot.in">support@sociobot.in</a>.</p>`}</article></main>${footer()}`;bindLinks();focusHeading();
}
